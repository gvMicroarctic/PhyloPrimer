...
#connected to Probe_design_pp.pl and Probe_check_pp.pl : cross-dimer calculation for primers + probe
...
while(defined(my $input = <IN>)) {
    ...
    foreach my $p (@pair) { #each pair
        $countPair++;
        my ($primer_inputF,$primer_inputR) = split(/\t/, $p);
        if ($countPair == 1) {
            print $file "\n>\t$primer_inputF\t$primer_inputR";
        } elsif ($countPair == 2) {
            print $file "\t$primer_inputR\n";
        }
