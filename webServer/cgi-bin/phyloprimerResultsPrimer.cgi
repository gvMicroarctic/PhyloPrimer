...
#connected to Primer_design_pp.pl : visualise primer design results
...
while (defined(my $input = <INFO>)) {
    ...
    if ($input =~ /^CONSENSUS/) {
        ...
        my @info = split(/\t/, $input);
        $tableBlast = $info[1];
    } elsif ($input =~ /^PIECHART_FR/) {
        my @info = split(/\t/, $input);
        $tablePieChartFR = $info[1];
        ...
        my $combined = $pairSeq{$pair}{'F'} . "-" . $pairSeq{$pair}{'R'};
        $pairInfo{$pair} .= $self{$pairSeq{$pair}{'F'}} . "!" . $self{$pairSeq{$pair}{'R'}} . "!" . $cross{$combined} . "!" . $hairpin{$pairSeq{$pair}{'F'}} . "!" . $hairpin{$pairSeq{$pair}{'R'}} . "!" . $sp; #add info for dG
    }
}
close(INFO);
...
$tablePieChartFR = "'" . $tablePieChartFR . "'"; #tablePieChartFR
...
foreach my $pair (sort {$a <=> $b} keys %pairInfo) { #print only 50 combinations
    ...
    if ($pair == 1) {
        ...
        $radio .= "<span class='prefixMain'><b class='prefix'>F$primerInfo{$pairSeq{$pair}{'F'}}{'POSITION'}-$primerInfo{$pairSeq{$pair}{'F'}}{'LEN'}-</b>$pairSeq{$pair}{'F'}<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b class='prefix'>R$primerInfo{$pairSeq{$pair}{'R'}}{'POSITION'}-$primerInfo{$pairSeq{$pair}{'R'}}{'LEN'}-</b>$pairSeq{$pair}{'R'}<br></span>";
    } else {
        last;
    }
}
            ...
            <ul id="navbar">
                ...
                <li class="main" id="dG_navbarLi"><a id="dG_navbar">&Delta;G &#9662;</a>
                    <ul class="dropdown">
                        ...
                        <li class="maindG" id="cross_navbarLi"><a id="cross_navbar">Cross dimers</a>
                        </li>
                        <li class="maindG" id="hair_navbarLi"><a id="hair_navbar">Hairpins</a></li>
                    </ul>
                </li>
                <li class="main" id="taxonomy_navbarLi"><a id="taxonomy_navbar">GENBANK BLAST &#9662;</a>
                    <ul class="dropdown1">
    ...
    <button id="add_taxon" onclick="addTax()">+</button>
        ...
        <div id="oligoInfo">
            ...
            <div id="consensus_intro"></div>
                ...
                <br>
                <br>
                <h3>The table reports the specifics for both the forward and reverse primer (<b>F</b> and <b>R</b>, respectively). More information about the &Delta;G and T<sub>m</sub> can be found in the manual. All the reported position at the oligo position at the 5’ end.</h3>
            ...
            <div id="taxonomy_info">
                ...
                <h3>The primers were blasted against the nt database which is a taxonomical comprehensive subset of the GenBank database. The pie chart shows the number of hits for each taxon that were matched by both the forward and reverse primers. Please keep in mind that the abundance of each taxon may be skewed towards the taxa that are more represented in the BLAST database.</h3>
                <div id="taxonomy_contentFR"></div>
                <br>
...
function searchEng() {
    ...
    \$(".searchTaxon").each(function(){ //all taxonomy for all the input fields
        ...
        taxClick = taxClick.replace(/'/g, "\\[sub1\\]"); //'
        taxClick = taxClick.replace(/`/g, "\\[sub12\\]"); //`
        filterTax.push(taxClick);
    });
    ...
    \$(document).ready(function () {
        ...
        tablePieChartFR = $tablePieChartFR;
        tableUser = $tableUser;
        ...
        if (differentPosA === 'no') {
            ...
            document.getElementById("consensus_intro").innerHTML = "<h3>Here is the consensus PhyloPrimer calculated and used for the primer design and where you can visualize the position of the <span class='highlightedTextF'>forward primer</span> and the <span class='highlightedTextR'>reverse primer</span>. If you did not select all the sequences in the dynamics tree, PhyloPrimer will have created two consensus sequences: one with the selected sequences (positive consensus), and one with all the others (negative consensus). Here you visualize the positive consensus and the bold and colored letters indicate the differences between the two consensuses. All the gaps are removed from the positive consensus and any bases surrounding an area where a gap region is present in the positive consensus but not in the negative is marked with <b id='e'>bold</b> characters. Vice-versa, any positive consensus base corresponding to a gap on the negative consensus is colored with <b id='c'>blue</b>. Where, for a certain position, the base between the two consensuses differs, that base is reported in <b id='a'>red</b>.</h3>";
        }
        ...
        var infoTable = "<table><tr><th>Oligo type</th><th>Sequence</th><th>Position</th><th>Length</th><th>GC%</th><th>T<sub>m (<sup>o</sup>C)</sub></th><th>Self Dimer &Delta;G (kcal/mol)</th><th>Hairpin &Delta;G (kcal/mol)</th><th>T<sub>a</sub> (<sup>o</sup>C)</th><th>Cross dimer &Delta;G (kcal/mol)</th></tr><tr><td>F</td><td>" + res[0] + "</td><td>" + res[1] + "</td><td>" + res[2] + "</td><td>" + res[3] + "</td><td>" + res[4] + "</td><td>" + res[5] + "</td><td>" + res[6] + "</td><td rowspan='2'>" + res[14] + "</td><td rowspan='2'>" + res[15] + "</td></tr><tr><td>R</td><td>" + res[7] + "</td><td>" + res[8] + "</td><td>" + res[9] + "</td><td>" + res[10] + "</td><td>" + res[11] + "</td><td>" + res[12] + "</td><td>" + res[13] + "</td></tr></table>";
        ...
        if (tableBlast === "none") {
            ...
            document.getElementById("taxonomy_contentFR").innerHTML = "<h3>The BLAST search against the nucleotide BLAST database did not produce any result for any of the PhyloPrimer primers.</h3>";
            tablePieChartSubFR = 'zero';
            tableBlastSub = 'zero';
        } else {
            tablePieChartSubFR = tablePieChartFR.match("<" + combinedTrim + ">(.*?)" + "<>"); //primer pair pie chart
    ...
    \$(document).ready(function () {
        ...
        \$("#self_navbar").click(function () {
            ...
            infodG = "<h2>&Delta;G - Self Dimers</h2>" + textSelf + "</br><span class='prefix1'><b>F" + res[1] + "-" + res[2] + "-</b>" + res[0] + "</span>" + res[16] + "</br><span class='prefix1'><b>R" + res[8] + "-" + res[9] + "-</b>" + res[7] + "</span>" + res[17] + "</br>";
        ...
        \$("#cross_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Cross dimers</h2>" + textCross + res[18] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'cross';
        })
        \$("#hair_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Hairpins</h2>" + textHair + "</br><span class='prefix1'><b>F" + res[1] + "-" + res[2] + "-</b>" + res[0] + "</span>" + res[19] + "</br><span class='prefix1'><b>R" + res[8] + "-" + res[9] + "-</b>" + res[7] + "</span>" + res[20] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'hair';
        })
        ...
        \$("#domain_navbar").click(function () {
            ...
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Domain');
            }
            place = 'domain';
        })
        \$("#phylum_navbar").click(function () {
            ...
            navbarDisplay('none', 'none', 'block', 'none', 'none');
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Phylum');
            }
            place = 'phylum';
        })
        \$("#class_navbar").click(function () {
            ...
            navbarDisplay('none', 'none', 'block', 'none', 'none');
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Class');
            }
            place = 'class';
        })
        \$("#order_navbar").click(function () {
            ...
            navbarDisplay('none', 'none', 'block', 'none', 'none');
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Order');
            }
            place = 'order';
        })
        \$("#family_navbar").click(function () {
            ...
            navbarDisplay('none', 'none', 'block', 'none', 'none');
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Family');
            }
            place = 'family';
        })
        \$("#genus_navbar").click(function () {
            ...
            navbarDisplay('none', 'none', 'block', 'none', 'none');
            if ((tablePieChartSubFR !== 'zero') && (tableBlastSub !== 'zero')) {
                taxonomyPage(tablePieChartSubFR, tableBlastSub, 'Genus');
            }
            place = 'genus';
        })
        ...
        \$("#userCheck_navbar").click(function () {
            ...
            if (tableUser === "none") {
                    ...
                    if (infoUserSub === null) {
                        ...
                        document.getElementById("userCheck_content").innerHTML = "<h3>You submitted " + infoUserData[0] + " DNA sequences which PhyloPrimer blasted against these primers. Of all the sequences, " + infoUserData[1] + " were hit from both the forward and the reverse primer (<b>F</b> and <b>R</b>, respectively). In the following table all the BLAST matches are reported for the sequences hit by both F and R.</h3>";
    ...
    //TAXONOMY PAGE
    function taxonomyPage(tablePieChartSubFR, tableBlastSub, rank) {
        rankPass = rank;
        if ((tablePieChartSubFR === null) && (tableBlastSub === null)) {
            document.getElementById("taxonomy_intro").innerHTML = "<h2>GenBank BLAST</h2>";
            document.getElementById("taxonomy_contentFR").style.height = "30px";
            document.getElementById("taxonomy_contentFR").innerHTML = "<h3>The BLAST search against the nucleotide BLAST database did not match any entry for either the forward or the reverse primer.</h3>";
            document.getElementById("taxonomy_table_mess").innerHTML = "";
            document.getElementById("taxonomy_table").innerHTML = "";
        } else if ((tablePieChartSubFR === null) && (tableBlastSub !== null)) {
            document.getElementById("taxonomy_intro").innerHTML = "<h2>GenBank BLAST - " + rank + "</h2>";
            document.getElementById("taxonomy_contentFR").style.height = "30px";
            document.getElementById("taxonomy_contentFR").innerHTML = "<h3>None of the entries in the nucleotide BLAST database matched with both the forward and the reverse primers.</h3>";
            tableBlastSubNew = tableBlastSub[1] + "</table>";
            document.getElementById("taxonomy_table_mess").innerHTML = "<h3>Whereas, the pie chart reports only the hits found for both the forward and reverse primers (<b>F</b> and <b>R</b>, respectively), in the following table all the BLAST matches are reported for the sequences hit by both F and R, the sequences hit by only F and the sequences hit by only R.</h3>";
            ...
            document.getElementById("taxonomy_intro").innerHTML = "<h2>GenBank BLAST - " + rank + "</h2>";
            document.getElementById("taxonomy_contentFR").style.height = "500px";
            document.getElementById("taxonomy_contentFR").style.width = document.getElementById("taxonomy_info").offsetWidth;
            tablePieChartDivFR = tablePieChartSubFR[1].split(":");
            renderChart("taxonomy_contentFR", "", tablePieChartDivFR, rank);
            var c = document.getElementsByClassName("canvasjs-chart-canvas")[0];
            var ctx = c.getContext("2d");
            ctx.fillStyle = 'white';
...
\$(document).ready(function () {
    \$(window).resize(function() {
        document.getElementById("taxonomy_contentFR").style.width = document.getElementById("taxonomy_info").offsetWidth;
        renderChart("taxonomy_contentFR", "", tablePieChartDivFR, rankPass);
        var c = document.getElementsByClassName("canvasjs-chart-canvas")[0];
        var ctx = c.getContext("2d");
        ctx.fillStyle = 'white';
        ctx.fillRect(0, 489, (document.getElementById("taxonomy_info").offsetWidth/5), 40);
...
function autocompleteDrop(id) {
    var text = id.value;
    text = text.replace(/\'/g, "\[sub1\]");
    text = text.replace(/\`/g, "\[sub12\]");
    \$.ajax({
        ...
        for (i = 0; i < (arr.length-1); i++) {
            /*check if the item starts with the same letters as the text field value:*/
                if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                    ...
                    arr[i] = arr[i].replace(/\\[sub1\\]/g, "&apos;"); //'
                    arr[i] = arr[i].replace(/\\[sub12\\]/g, "&grave;"); //`
