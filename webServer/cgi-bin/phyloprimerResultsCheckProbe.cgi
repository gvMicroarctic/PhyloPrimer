...
#connected to Probe_check_pp.pl : visualise probe check results
...
while (defined(my $input = <INFO>)) {
    ...
    if ($input =~ /^BLAST_TABLE/) {
        ...
        if ($probe =~ /-/) {
            ($probe) = $pairSeq{$pair}{'P'} =~ /-(.*)/;
        }
        my $combined = $for . "-" . $rev . "-" . $probe;
        $pairInfo{$pair} .= $self{$for} . "!" . $self{$rev} . "!" . $self{$probe} . "!" . $cross{$combined} . "!" . $hairpin{$for} . "!" . $hairpin{$rev} . "!" . $hairpin{$probe} . "!" . $sp; #add info for dG
    }
}
close(INFO);
...
foreach my $pair (sort {$a <=> $b} keys %pairInfo) { #print only 50 combinations
    if ($pair == 1) {
        ...
        $radio .= "<span class='prefixMain'>$pairSeq{$pair}{'F'}<br>&nbsp;&nbsp;&nbsp;&nbsp;$pairSeq{$pair}{'R'}<br>&nbsp;&nbsp;&nbsp;&nbsp;$pairSeq{$pair}{'P'}<br></span>";
    } elsif ($pair <= 100) {
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="oligoList">
            ...
            <h2>$project</h2>
            <div id="oligoList_intro"><h3>Please select the primer pair and probe you want to explore from the list below. All the information can be also found in the archive which is downloadable from the Archive window. Use the following research section for highlighting the oligos that reflect certain criteria:</h3></div>
...
$radio
        ...
        <div id="oligoInfo">
            <div id="consensus_info">
                <h2>Specifics</h2>
                <h3>The table reports the specifics for both the forward and reverse primer and the probe (<b>F</b>, <b>R</b> and <b>P</b>, respectively). More information about the &Delta;G and T<sub>m</sub> can be found in the manual.</h3>
            ...
            <div id="taxonomy_info">
                ...
                <h3>These oligos were blasted against the nt database which is a taxonomical comprehensive subset of the GenBank database. The pie chart shows the number of hits for each taxon that were matched by both the forward and reverse primers and the probe. Please keep in mind that the abundance of each taxon may be skewed towards the taxa that are more represented in the BLAST database.</h3>
...
function searchEng() {
    ...
    \$(document).ready(function () {
        ...
        \$("#self_navbar").click(function () {
            ...
            infodG = "<h2>&Delta;G - Self Dimers</h2>" + textSelf + "</br>" + res[0] + res[20] + "</br>" + res[6] + res[21] + "</br>" + res[12] + res[22] + "</br>";
        ...
        \$("#cross_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Cross dimers</h2>" + textCross + res[23] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'cross';
        })
        \$("#hair_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Hairpins</h2>" + textHair + "</br>" + res[0] + res[24] + "</br>" + res[6] + res[25] + "</br>" + res[12] + res[26] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'hair';
        })
