...
#connected to Primer_design_pp.pl, Probe_design_pp.pl and Oligo_design_pp.pl: perform blast + bowtie search on PhyloPrimer designed oligos
...
while (defined(my $input = <FILEIN>)) {
    chomp($input);
    my ($pair, $seq, $len, $start, $type) = split(/\t/, $input);
    $oligo{$len}{$type}{$seq} = $start;
...
if ($typeAll ne 'oligo') { #probe or primer
    ...
    if ($type eq 'nt') {
        ...
        foreach my $len (sort {$b <=> $a} keys %oligo) {
            foreach my $seq (sort keys %{$oligo{$len}{'for'}}) {
                ...
                foreach my $countCl (keys %cluster) {
                    if (($start >= $cluster{$countCl}{'start'}) && ($end <= $cluster{$countCl}{'end'})) {
                        $cluster{$countCl}{'seq'}{$seq} = '';
                        $oligoCl{$seq} = $countCl;
                        $inside = 1;
                        if ($start < $cluster{$countCl}{'startReal'}) {
                            $cluster{$countCl}{'startReal'} = $start;
                        }
                        if ($end > $cluster{$countCl}{'endReal'}) {
                            $cluster{$countCl}{'endReal'} = $end;
                        }
                        last;
                    }
                }
                if ($inside == 0) {
                    $cluster{$count}{'start'} = $start - 5;
                    $cluster{$count}{'end'} = $end + 5;
                    $cluster{$count}{'type'} = 'for';
                    $cluster{$count}{'startReal'} = $start;
                    $cluster{$count}{'endReal'} = $end;
                    $oligoCl{$seq} = $count;
                }
            }
        }
        ...
        foreach my $countCl (keys %cluster) {
            if ($cluster{$countCl}{'type'} eq 'for') {
                $forCount++;
                my $start = $cluster{$countCl}{'startReal'} - 1;
                my $diff = $cluster{$countCl}{'endReal'} - $cluster{$countCl}{'startReal'} + 3;
                my $seq = substr $consensus, $start, $diff;
                print $file ">${countCl}\n";
                print $file "$seq\n";
            }
        }
        ...
        while (defined(my $input = <FILEIN>)) {
            ...
            if ($info[8] < $info[9]) {
                $m8{$info[0]}{$info[1]}{$pos} = '+'; #cluster - accession - position = sense
        ...
        while (defined(my $input = <FILEIN>)) {
            ...
            if ($type eq 'for') {
                ...
                $rev = $seq;
                foreach my $acc (keys %{$m8{$oligoCl{$for}}}) {
                    if (defined($m8{$oligoCl{$rev}})) {
                        foreach my $posFor (keys %{$m8{$oligoCl{$for}}{$acc}}) {
                            foreach my $posRev (keys %{$m8{$oligoCl{$rev}}{$acc}}) {
                                my $diff = $posFor - $posRev;
                                #                                if ($m8{$oligoCl{$for}}{$acc}{$posFor} ne $m8{$oligoCl{$rev}}{$acc}{$posRev}) { #opposite directions - sense and antisense
    ...
    foreach my $pair (sort {$a <=> $b} keys %oligoPair) {
        ...
        foreach my $acc (keys %{$score{$pair}}) {
            ...
            print $file "$mis\t$oligoPair{$pair}{'oligo'}\t$acc\t$alignment\t$score{$pair}{$acc}{'startOligo'}\t$score{$pair}{$acc}{'endOligo'}\n";
        }
    }
    close($file);
}
