...
#connected to Primer_check_pp.pl : visualise primer check results
...
while (defined(my $input = <INFO>)) {
    ...
    if ($input =~ /^BLAST_TABLE/) {
        ...
        my $combined = $for . "-" . $rev;
        $pairInfo{$pair} .= $self{$for} . "!" . $self{$rev} . "!" . $cross{$combined} . "!" . $hairpin{$for} . "!" . $hairpin{$rev} . "!" . $sp; #add info for dG
    }
}
close(INFO);
...
foreach my $pair (sort {$a <=> $b} keys %pairInfo) { #print only 50 combinations
    if ($pair == 1) {
        ...
        $radio .= "<span class='prefixMain'>$pairSeq{$pair}{'F'}<br>&nbsp;&nbsp;&nbsp;&nbsp;$pairSeq{$pair}{'R'}<br></span>";
    } elsif ($pair <= 100) {
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="oligoList">
            ...
            <h2>$project</h2>
            <div id="oligoList_intro"><h3>Please select the primer pair you want to explore from the list below. All the information can be also found in the archive which is downloadable from the Archive window. Use the following research section for highlighting the oligos that reflect certain criteria:</h3></div>
...
$radio
        ...
        <div id="oligoInfo">
            ...
            <div id="consensus_intro"></div>
                <h3>The table reports the specifics for both the forward and reverse primers (<b>F</b> and <b>R</b>, respectively). More information about the &Delta;G and T<sub>m</sub> can be found in the manual.</h3>
...
function searchEng() {
    ...
    for (i = 0; i < nodes.length; i++) {
        ...
        console.log(res[4] + " and " + res[11] + " and " + res[2] + " and " + res[9]);
        
        if ((res[3] >= filterTm_min) && (res[3] <= filterTm_max) && (res[9] >= filterTm_min) && (res[9] <= filterTm_max) && (res[1] >= filterLen_min) && (res[1] <= filterLen_max) && (res[7] >= filterLen_min) && (res[7] <= filterLen_max)) {
            var inside = 1;
            if (filterTax[0] !== '') {
    ...
    \$(document).ready(function () {
        ...
        \$("#self_navbar").click(function () {
            ...
            infodG = "<h2>&Delta;G - Self Dimers</h2>" + textSelf + "</br>" + res[0] + res[14] + "</br>" + res[6] + res[15] + "</br>";
        ...
        \$("#cross_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Cross dimers</h2>" + textCross + res[16] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'cross';
        })
        \$("#hair_navbar").click(function () {
            ...
            navbarDisplay('none', 'block', 'none', 'none', 'none');
            infodG = "<h2>&Delta;G - Hairpins</h2>" + textHair + "</br>" + res[0] + res[17] + "</br>" + res[6] + res[18] + "</br>";
            document.getElementById("dG_content").innerHTML = infodG;
            place = 'hair';
        })
