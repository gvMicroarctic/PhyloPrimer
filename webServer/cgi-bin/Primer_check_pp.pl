...
#connected to backgroundDesignCheck.pl : primer check pipeline
...
#Usage: ./Primer_check_pp.pl -folder $path_html/analysesPhyloprimer/BZpTYlKPtufEzMrMRTylCa -file BZpTYlKP.info4
...
close($tmp);
`$path_cgi/Cross_dimer_checking_web_pp.pl -folder $folder -primer Cross_1.tmp -mg $mg_tot -mon $monovalent -oligo $C -dntp $dNTP_tot -t $temperature_celsius`;
...
if (-z $checkFile) { #if file is empty
    ...
    foreach my $pair (sort {$a <=> $b} keys %checkPaired) {
        ...
        my $combined = $checkPaired{$pair}{'F'}{'TITLE'} . "-" . $checkPaired{$pair}{'R'}{'TITLE'};
        ...
        foreach my $mis (@mismatch) { #print first alignments with less mismatches
            foreach my $acc (sort {$tableBlast{$pair}{$mis}{$a}{'MATCH'} <=> $tableBlast{$pair}{$mis}{$b}{'MATCH'}} keys %{$tableBlast{$pair}{$mis}}) {
                ...
                if ($taxonomy{$acc}{'DOMAIN'} eq "") {
                    ...
                    $tableFR .= "<td rowspan='2' class ='D'>" . $taxonomy{$acc}{'DOMAIN'} . "</td>";
                }
        ...
        if (defined($pieFR{'DOMAIN'})) {
            $pieChartTableFR .= "<" . $combined . ">";
...
my $message = "Hi,\n\nPlease find the link to the PhyloPrimer results: https:\/\/www.cerealsdb.uk.net\/cerealgenomics\/cgi-bin\/phyloprimerResultsCheckPrimer.cgi?defSet=" . $defSet . "\n\n\nAll the best,\nPhyloPrimer team";
