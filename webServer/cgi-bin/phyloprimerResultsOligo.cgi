...
#connected to Oligo_design_pp.pl : visualise oligo design results
...
while (defined(my $input = <INFO>)) {
    ...
    if ($input =~ /^CONSENSUS/) {
        ...
        $pairInfo{$pair} .= $self{$pairSeq{$pair}{'O'}} . "!" . $hairpin{$pairSeq{$pair}{'O'}} . "!" . $sp; #add info for dG
    }
}
close(INFO);
...
foreach my $pair (sort {$a <=> $b} keys %pairInfo) { #print only 50 combinations
    if ($pair == 1) {
        ...
        $radio .= "<span class='prefixMain'><b class='prefix'>$primerInfo{$pairSeq{1}{'O'}}{'POSITION'}-$primerInfo{$pairSeq{1}{'O'}}{'LEN'}-</b>$pairSeq{$pair}{'O'}<br></span>";
    } elsif ($pair <= 100) {
        $radio .= "<input type='radio' name='pair' class='pair' value='$pairInfo{$pair}'>";
        $radio .= "<span class='prefixMain'><b class='prefix'>$primerInfo{$pairSeq{$pair}{'O'}}{'POSITION'}-$primerInfo{$pairSeq{$pair}{'O'}}{'LEN'}-</b>$pairSeq{$pair}{'O'}<br></span>";
    } else {
        last;
    }
}
...
<br>
<br>
<h3>The table reports the specifics for selected oligo. More information about the &Delta;G and
T<sub>m</sub> can be found in the manual. All the reported position at the oligo position at the
5’ end.</h3>
...
<h3>The oligos were blasted against the nt database which is a taxonomical comprehensive subset of
the GenBank database. The pie chart shows the number of hits for each taxon that were matched by
the selected oligo. Please keep in mind that the abundance of each taxon may be skewed towards
the taxa that are more represented in the BLAST database.</h3>
...
<br>
<li><b>inputs</b> which contains your input files and files PhyloPrimer used for the primer design (more
details on
...
\$(document).ready(function () {
    ...
    var infoTable = "<table><tr><th>Sequence</th><th>Position</th><th>Strand</th><th>Length</th><th>GC%</th><th>T<sub>m (<sup>o</sup>C)</sub></th><th>Self Dimer &Delta;G (kcal/mol)</th><th>Hairpin &Delta;G (kcal/mol)</th></tr><tr><td>" + res[0] + "</td><td>" + res[1] + "</td><td>" + res[5] + "</td><td>" + res[2] + "</td><td>" + res[3] + "</td><td>" + res[4] + "</td><td>" + res[6] + "</td><td>" + res[7] + "</td></tr></table>";
    ...
    if (tableBlast === "none") {
        ...
        document.getElementById("taxonomy_contentF").innerHTML = "<h3>The BLAST search against the nucleotide BLAST database did not produce any result for any of the PhyloPrimer oligos.</h3>";
...
\$(document).ready(function () {
    ...
    \$("#self_navbar").click(function () {
        ...
        infodG = "<h2>&Delta;G - Self Dimers</h2>" + textSelf + res[8] + "</br>";
    ...
    \$("#hair_navbar").click(function () {
        ...
        navbarDisplay('none', 'block', 'none', 'none', 'none');
        infodG = "<h2>&Delta;G - Hairpins</h2>" + textHair + res[9] + "</br>";
        document.getElementById("dG_content").innerHTML = infodG;
        place = 'hair';
    })
...
\$('input:radio[name=pair]').change(function () {
    ...
    if (place === 'consensus') {
        ...
        infodG = "<h2Self Dimers</h2>" + textSelf + "</br>" + res[8] + "</br>";
        document.getElementById("dG_content").innerHTML = infodG;
    } else if (place === 'hair') {
        infodG = "<h2Hairpins</h2>" + textHair + "</br>" + res[9] + "</br>";
        document.getElementById("dG_content").innerHTML = infodG;
...
function taxonomyPage(tablePieChartSubF, tableBlastSub, rank) {
    ...
    if ((tablePieChartSubF === null) && (tableBlastSub === null)) {
        ...
        document.getElementById("taxonomy_contentF").style.height = "30px";
        document.getElementById("taxonomy_contentF").innerHTML = "<h3>The BLAST search against the nucleotide BLAST database did not match any entry for any of the oligos.</h3>";
        ...
        document.getElementById("taxonomy_contentF").style.height = "30px";
        document.getElementById("taxonomy_contentF").innerHTML = "<h3>None of the entries in the nucleotide BLAST database matched any of the oligos.</h3>";
